...
df = pd.read_csv('Data/val_data.csv', encoding="latin1", engine="python",
                            on_bad_lines="skip")
...
df['cleaned_text'] = df['post'].apply(preprocess)  # Apply preprocess to 'cleaned_text' column
df['tokens'] = df['cleaned_text'].apply(tokenize)  # Apply tokenize to 'cleaned_text' column
df.drop(columns=['post'], inplace=True)

# Save the resulting DataFrame to a new CSV
df.to_csv('Data/tokenized/tokens_val_data.csv', index=False)
